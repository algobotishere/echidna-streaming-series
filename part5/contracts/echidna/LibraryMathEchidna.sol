...
contract LibraryMathEchidna {
  ...
  function reserve_allocate(uint256 delRisky, uint256 delStable)
    public
    returns (Reserve.Data memory preAllocateReserves, uint256 delLiquidity)
  {
    //************************* Pre-Conditions *************************/
  ...
  function allocate_then_remove(uint256 delRisky, uint256 delStable) public {
    ...
    (Reserve.Data memory preAllocateReserves, uint256 delAllocateLiquidity) = reserve_allocate(
      delRisky,
      delStable
    ); // this allocates to the pool, this ensures reserve balance increases
    ...
    if (preAllocateReserves.reserveStable != reserve.reserveStable) {
      emit AssertionFailed(
        "preallocate stable not equivalent to current stable",
    ...
    if (preAllocateReserves.liquidity != reserve.liquidity) {
      emit AssertionFailed(
        "preallocate liq not equivalent to current liq",
