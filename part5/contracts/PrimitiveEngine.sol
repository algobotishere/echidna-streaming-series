...
contract PrimitiveEngine is IPrimitiveEngine {
  ...
  constructor() {
    (
      ...
      scaleFactorStable,
      MIN_LIQUIDITY
    ) = IPrimitiveFactory(msg.sender).args();
  }
  ...
  function updateLastTimestamp(bytes32 poolId)
    external
    override
    lock
    returns (uint32 lastTimestamp)
  {
  ...
  function invariantOf(bytes32 poolId) public view override returns (int128 invariant) {
    ...
    (uint256 riskyPerLiquidity, uint256 stablePerLiquidity) = reserves[poolId].getAmounts(
      PRECISION
    ); // 1e18 liquidity
